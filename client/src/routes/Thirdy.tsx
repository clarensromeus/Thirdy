...
import {
  RandomFriendRequestQuery,
  RandomFriendRequestQueryVariables,
...
const Thirdy = (): JSX.Element => {
  ...
  const { data } = useQuery<
    ...
    RandomFriendRequestQueryVariables
  >(RANDOM_FRIEND_REQUEST, {
    variables: { acceptedId: `${AuthInfo.Data?._id}` },
  });

  // follow friend back mutation
  const [followFriendBack, { loading: followbackLoading }] = useMutation<
    FollowBackMutation,
    FollowBackMutationVariables
  >(FOLLOW_FRIENDS_BACK);
    ...
    <>
      ...
      <Box
        sx={{
          ...
          gap: 5,
          pt: { xs: 2, sm: 2, lg: 3 },
          pb: { xs: 4.4, sm: 4.4, lg: 2 },
        }}
      >
        <Box
          sx={{
            width: width && width < 720 ? "100%" : "max(20%, 70%)",
            display: "flex",
        ...
        >
          <Paper
            elevation={isEqual(mode.mode, "light") ? 1 : 5}
            sx={{ borderRadius: 2 }}
          >
            <Box
              className="stories"
          ...
          <Paper
            sx={{ borderRadius: 2 }}
            elevation={isEqual(mode.mode, "light") ? 1 : 6}
          >
            <Box
              py={2}
              sx={{
          ...
          <Box>
            <Box
              ...
              sx={{
                position: "sticky",
                top: "80px",
                mx: isNil(data?.randomFriendRequest?.RequestId) ? 1.4 : 0,
              }}
            >
              {isNil(data?.randomFriendRequest?.RequestId) ? (
                <Sponsors />
              ) : (
                <React.Fragment>
                  <Box>
                  ...
                  {wait ? (
                    ...
                    <Box sx={{ display: "flex", gap: 2, py: 2 }}>
                      <Box>
                        <Avatar
                          alt="friendRequestImage"
                          src={`${data?.randomFriendRequest?.User?.Image}`}
                        />
                      </Box>
                      <Box
                        sx={{
                      ...
                      >
                        ...
                        >
                          <Typography
                            ...
                            color={
                              isEqual(mode.mode, "light") ? "black" : "white"
                            }
                          >
                            {data?.randomFriendRequest?.User?.Firstname?.charAt(
                              0
                            )}
                            . {data?.randomFriendRequest?.User?.Lastname}
                          </Typography>
                          <Typography
                            fontSize="13px"
                            fontWeight="600"
                          ...
                          >
                            {slice(
                              dayjs(`${data?.randomFriendRequest?.createdAt}`)
                                .fromNow(true)
                                .replace(/^a/, "1"),
                              0,
                              -3
                        ...
                        <Box sx={{ display: "flex", gap: 1 }}>
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                await followFriendBack({
                                  variables: {
                                    FriendId: data?.randomFriendRequest?._id,
                                    AcceptedId: `${AuthInfo.Data?._id}`,
                                    userRequestId:
                                      data?.randomFriendRequest?.User?._id,
                                  },
                                  refetchQueries: [
                                    RANDOM_FRIEND_REQUEST,
                                    USER_CHATS,
                                  ...
                                  onCompleted: async () => {
                                    ...
                                    await CreateNotification({
                                      ReceiverId: `${data?.randomFriendRequest?.User?._id}`,
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                await RejectRequest({
                                  variables: {
                                    rejectRequestFriendId:
                                      data?.randomFriendRequest?._id,
                                  },
                                  onCompleted: async () => {
                                    try {
                                      await CreateNotification({
                                  ...
                                  refetchQueries: [RANDOM_FRIEND_REQUEST],
                                });
                              } catch (error) {
                                throw new Error(`${error}`);
                              }
              ...
              >
                ...
                <Box mr={isNil(data?.randomFriendRequest?.RequestId) ? 1.3 : 0}>
                  <IconButton
                    sx={{ mt: 1, px: 0 }}
                    disableRipple
                    disableFocusRipple
