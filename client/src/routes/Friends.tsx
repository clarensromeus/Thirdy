...
const Friends = () => {
  ...
  const [follow, { data: followData, loading: followLoading }] = useMutation<
    FollowMutation,
    FollowMutationVariables
  >(FOLLOW_FRIENDS);

  const [followBack, { data: followBackData, loading: followBackLoading }] =
    useMutation<FollowBackMutation, FollowBackMutationVariables>(
      FOLLOW_FRIENDS_BACK
    );

  const [unfollow, { data: unfollowData, loading: unfollowLoading }] =
    useMutation<UnFollowMutation, UnFollowMutationVariables>(UNFOLLOW_FRIENDS);

  const [rejectRequest, { data: rejectData, loading: rejectLoading }] =
    useMutation<RejectRequestMutation, RejectRequestMutationVariables>(
      REJECT_REQUEST
    );
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <Box
                      sx={{ alignSelf: "flex-start", display: "flex", gap: 1 }}
                    >
                      <AvatarGroup max={2}>
                        <Avatar
                          alt="Remy Sharp"
