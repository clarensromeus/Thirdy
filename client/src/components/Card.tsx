...
export default function CardPost({ width }: ICard) {
  ...
  return (
    <>
      ...
      {loading && isUndefined(data?.AllPosts?.Posts) ? (
        <Box
          sx={{
            maxWidth: width,
            display: "flex",
        ...
        data?.AllPosts?.Posts?.map((value, index) => {
          const {
            _id,
            PostId,
            PostImage,
          ...
          return (
            <React.Fragment key={index}>
              ...
              {index === size(data.AllPosts?.Posts) - 1 &&
                isEqual(Boolean(data.AllPosts.hasNextPage), true) && (
                  <Waypoint
                    onEnter={() =>
                      fetchMore({
                        variables: {
                          cursor: data.AllPosts.cursor,
                          limit: 3,
                        },
                        /* updateQuery(prev, { fetchMoreResult }) {
                          ...
                          const previousResult = get(prev, DATA_PATH); // like prev.GetAllPosts.PostInfo
                          const nextResult = get(fetchMoreResult, DATA_PATH); // like fetchMore.GetAllPost.PostInfo
                          // merge the previous and the next post
                          let result = setWith(
                            clone(prev),
                            DATA_PATH,
                            merge(previousResult, nextResult),
                            clone
                          );
                          return result;
                        }, */
              ...
              {networkStatus === 3 && Boolean(data.AllPosts.hasNextPage) && (
                <Box
                  sx={{
                    maxWidth: width,
                    display: "flex",
